...
$Script:JsonLogFile = Join-Path $env:USERPROFILE ".claude-orchestrator\usage\logs\rotation-codex.jsonl"

# Optional structured logger (Task 2). Used by Read-CooldownFile to emit
# 'corrupt-cooldown' events. Falls back to plain Write-Host if missing.
...
function Write-Log {
    ...
    # Mirror to rotation-codex.jsonl as a generic 'log' event for tooling.
    if ($Script:HasStructuredLogger) {
        ...
        try {
            Write-StructuredLog -Path $Script:JsonLogFile -Event 'log' -Level $structuredLevel -Properties @{
                msg = $Message
                cli = 'codex'
            }
        } catch {
    ...
    try {
        ...
        if ($Script:HasStructuredLogger) {
            ...
            try {
                ...
                # silent: fallback do Read-CooldownFile (codex) sem structured logger; se Add-Content falhar ja emitimos Write-Host plaintext acima
                $null = $_
            }
        }
        return $null
...
function Get-ProfileUsagePct {
    ...
    # BUG-C: Read-CooldownFile loga eventos estruturados em caso de corrupcao
    # em vez do catch silencioso original.
    $cooldownPath = Join-Path $ProfilesRoot "$ProfileName\.cooldown"
    if (Test-Path -LiteralPath $cooldownPath) {
        $until = Read-CooldownFile -Path $cooldownPath
        if ($null -ne $until) {
...
if ($Script:HasStructuredLogger) {
    try {
        ...
        # silent: structured logger best-effort para evento 'rotate-decision' (codex); logging nao deve interromper rotacao
        $null = $_
    }
}
...
foreach ($prof in $allProfiles) {
    ...
    # BUG-C: Read-CooldownFile loga 'corrupt-cooldown' em vez de catch silencioso.
            ...
            if ($Script:HasStructuredLogger) {
                try {
                    Write-StructuredLog -Path $Script:JsonLogFile -Event 'silent-catch' -Level 'warn' -Properties @{
                        location = 'auto-rotate-codex.ps1:344'
                        reason   = $_.Exception.Message
                        profile  = $prof
                        note     = 'usage-snapshot-parse-failed-treating-profile-as-available'
                    }
                } catch {
...
if ($Script:HasStructuredLogger) {
    try {
        Write-StructuredLog -Path $Script:JsonLogFile -Event 'cooldown-set' -Level 'info' -Properties @{
            ...
            cooldownUntil  = (ConvertFrom-UnixTimestamp -Timestamp $resetsAt).ToString('o')
            window         = $triggerLimit
            resetsAtUnix   = [long]$resetsAt
            fallback       = (-not $usageData.resetsAt -or [long]$usageData.resetsAt -le 0)
        }
    } catch {
        # silent: structured logger best-effort para evento 'cooldown-set' (codex); logging nao deve interromper rotacao
        $null = $_
    }
}
...
# Structured event: actual rotation applied (auth.json swapped in ~/.codex)
if ($Script:HasStructuredLogger) {
    try {
        Write-StructuredLog -Path $Script:JsonLogFile -Event 'rotate' -Level 'info' -Properties @{
            ...
            window        = $triggerLimit
            triggerReason = if ($Force) { 'manual-force' } else { 'threshold' }
            swapMethod    = 'copy-auth-json'
        }
    } catch {
        # silent: structured logger best-effort para evento 'rotate' (codex); logging nao deve interromper rotacao
        $null = $_
    }
}
